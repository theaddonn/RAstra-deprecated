...
pub async fn handle_packs(connection_write_half: &ConnectionWriteHalf, conn_info: &mut ConnInfo) -> Result<(), RastraError> {
    ...
    let data = encode(vec![writer.get_payload(GamePacket::ResourcePacksInfo)], conn_info).await.unwrap();
    ...
    let data = encode(vec![writer.get_payload(GamePacket::ResourcePackStack)], conn_info).await.unwrap();
