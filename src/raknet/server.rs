...
use crate::raknet::packet::*;
use crate::raknet::utils::*;
use crate::{raknet::socket::*, raknet_log_debug, raknet_log_error};
...
impl RaknetListener {
    ...
    /// ```
    pub async fn bind(sockaddr: &SocketAddr) -> ServerResult<Self> {
        let s = match UdpSocket::bind(sockaddr).await {
            Ok(p) => p,
            Err(_) => {
                return Err(Error::RAKNET_BindAdressError);
            }
        };
    ...
    /// ```
    pub async fn from_std(s: std::net::UdpSocket) -> ServerResult<Self> {
        ...
        let s = match UdpSocket::from_std(s) {
            Ok(p) => p,
            Err(_) => {
                return Err(Error::RAKNET_SetRaknetRawSocketError);
            }
        };
    ...
    pub async fn listen(&mut self) {
        ...
        if self.motd.is_empty() {
            self.set_pong_data(
                Vec::from("MCPE;Dedicated Server;390;1.0;0;10;13253860892328930865;Bedrock level;Survival;1;19132;19133;".as_bytes())
            ).expect("Error changing empty motd in listen");
        }
        ...
        let motd = self.get_pong_data().await;
            ...
            loop {
                ...
                match cur_status {
                    PacketID::UnconnectedPing1 => {
                        ...
                        let packet = crate::raknet::packet::PacketUnconnectedPong {
                    ...
                    PacketID::OpenConnectionRequest1 => {
                        ...
                        {
                            let packet = crate::raknet::packet::IncompatibleProtocolVersion {
                        ...
                        let packet = crate::raknet::packet::OpenConnectionReply1 {
                            magic: true,
                            guid,
                    ...
                    PacketID::OpenConnectionRequest2 => {
                        ...
                        let packet = crate::raknet::packet::OpenConnectionReply2 {
                            magic: true,
                            guid,
                            address: addr,
                            mtu: req.mtu,
    ...
    /// ```
    pub async fn accept(&mut self) -> ServerResult<RaknetSocket> {
        if !self.listened {
            Err(Error::RAKNET_NotListen)
        } else {
            tokio::select! {
    ...
    /// ```
    pub async fn get_pong_data(&self) -> Vec<u8> {
        self.motd.clone()
    }
    ...
    /// ```
    pub fn set_pong_data(&mut self, motd: Vec<u8>) -> ServerResult<()> {
        self.motd = motd;
        Ok(())
    }

    pub async fn get_peer_raknet_version(&self, peer: &SocketAddr) -> ServerResult<u8> {
