...
impl PacketID {
    ...
    pub fn from(id: u8) -> ServerResult<Self> {
        ...
        match id {
            ...
            _ => Err(Error::RAKNET_IncorrectPacketID),
        }
    }
}
...
pub fn read_packet_ping(buf: &[u8]) -> ServerResult<PacketUnconnectedPing> {
    let mut cursor = PacketReader::new(buf.to_vec());
...
pub fn write_packet_ping(packet: &PacketUnconnectedPing) -> ServerResult<Vec<u8>> {
    let mut cursor = PacketWriter::new();
...
pub fn read_packet_pong(buf: &[u8]) -> ServerResult<PacketUnconnectedPong> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
    Ok(PacketUnconnectedPong {
        ...
        guid: unwrap_or_return!(cursor.read_u64(Endian::Big)),
        magic: unwrap_or_return!(cursor.read_magic()),
        motd: Vec::from(unwrap_or_return!(cursor.read_string()).as_bytes()),
    })
}

pub fn write_packet_pong(packet: &PacketUnconnectedPong) -> ServerResult<Vec<u8>> {
    let mut cursor = PacketWriter::new();
    unwrap_or_return!(cursor.write_u8(PacketID::UnconnectedPong.to_u8()));
    unwrap_or_return!(cursor.write_i64(packet.time, Endian::Big));
    unwrap_or_return!(cursor.write_u64(packet.guid, Endian::Big));
    unwrap_or_return!(cursor.write_magic());
    unwrap_or_return!(cursor.write_string(&*String::from_utf8_lossy(&*packet.motd)));
    Ok(cursor.get_raw_payload())
}

pub fn read_packet_connection_open_request_1(buf: &[u8]) -> ServerResult<OpenConnectionRequest1> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
...
pub fn write_packet_connection_open_request_1(
...
pub fn read_packet_connection_open_request_2(buf: &[u8]) -> ServerResult<OpenConnectionRequest2> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
...
pub fn write_packet_connection_open_request_2(
...
pub fn read_packet_connection_open_reply_1(buf: &[u8]) -> ServerResult<OpenConnectionReply1> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
...
pub fn write_packet_connection_open_reply_1(
...
pub fn read_packet_connection_open_reply_2(buf: &[u8]) -> ServerResult<OpenConnectionReply2> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
...
pub fn write_packet_connection_open_reply_2(
...
pub fn _read_packet_already_connected(buf: &[u8]) -> ServerResult<AlreadyConnected> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
    Ok(AlreadyConnected {
        magic: unwrap_or_return!(cursor.read_magic()),
...
pub fn write_packet_already_connected(packet: &AlreadyConnected) -> ServerResult<Vec<u8>> {
    let mut cursor = PacketWriter::new();
...
pub fn read_packet_incompatible_protocol_version(
    buf: &[u8],
) -> ServerResult<IncompatibleProtocolVersion> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
...
pub fn read_packet_nack(buf: &[u8]) -> ServerResult<Nack> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
...
pub fn write_packet_nack(packet: &Nack) -> ServerResult<Vec<u8>> {
    let mut cursor = PacketWriter::new();
...
pub fn read_packet_ack(buf: &[u8]) -> ServerResult<Ack> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
    let record_count = unwrap_or_return!(cursor.read_u16(Endian::Big));
    let sequences = {
...
pub fn write_packet_ack(packet: &Ack) -> ServerResult<Vec<u8>> {
    let mut cursor = PacketWriter::new();
...
pub fn read_packet_connection_request(buf: &[u8]) -> ServerResult<ConnectionRequest> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
...
pub fn write_packet_connection_request(packet: &ConnectionRequest) -> ServerResult<Vec<u8>> {
    let mut cursor = PacketWriter::new();
...
pub fn read_packet_connection_request_accepted(
    buf: &[u8],
) -> ServerResult<ConnectionRequestAccepted> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
...
pub fn read_packet_new_incomming_connection(buf: &[u8]) -> ServerResult<NewIncomingConnection> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
...
pub fn write_packet_new_incomming_connection(
...
pub fn read_packet_connected_ping(buf: &[u8]) -> ServerResult<ConnectedPing> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
    Ok(ConnectedPing {
...
pub fn write_packet_connected_ping(packet: &ConnectedPing) -> ServerResult<Vec<u8>> {
    let mut cursor = PacketWriter::new();
...
pub fn _read_packet_connected_pong(buf: &[u8]) -> ServerResult<ConnectedPong> {
    let mut cursor = PacketReader::new(buf.to_vec());
    unwrap_or_return!(cursor.read_u8());
    Ok(ConnectedPong {
        client_timestamp: unwrap_or_return!(cursor.read_i64(Endian::Big)),
...
pub fn write_packet_connected_pong(packet: &ConnectedPong) -> ServerResult<Vec<u8>> {
    let mut cursor = PacketWriter::new();
