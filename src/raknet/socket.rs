...
use crate::raknet::{arq::*, packet::*, utils::*};
...
impl RaknetSocket {
    ...
    pub async fn connect_with_version(addr: &SocketAddr, raknet_version: u8) -> ServerResult<Self> {
        let guid: u64 = rand::random();

        let s = match UdpSocket::bind("0.0.0.0:0").await {
            Ok(p) => p,
            Err(_) => return Err(Error::RAKNET_BindAdressError),
        };
            ...
            if reply1_buf[0] != PacketID::OpenConnectionReply1.to_u8() {
                if reply1_buf[0] == PacketID::IncompatibleProtocolVersion.to_u8() {
                    let _packet = match read_packet_incompatible_protocol_version(&buf[..size]) {
                        Ok(p) => p,
                        Err(_) => return Err(Error::RAKNET_NotSupportVersion),
                    };

                    return Err(Error::RAKNET_NotSupportVersion);
                } else {
        ...
        let reply1 = match read_packet_connection_open_reply_1(&reply1_buf[..reply1_size]) {
            Ok(p) => p,
            Err(_) => return Err(Error::RAKNET_PacketParseError),
        };
    ...
    /// assert!((0..10).contains(&latency));
    /// ```
    pub async fn ping(addr: &SocketAddr) -> ServerResult<(i64, String)> {
        let s = match UdpSocket::bind("0.0.0.0:0").await {
            Ok(p) => p,
            Err(_) => return Err(Error::RAKNET_BindAdressError),
        };
        ...
        loop {
            ...
            match s.send_to(buf.as_slice(), addr).await {
                ...
                Err(e) => {
                    raknet_log_error!("udp socket sendto error {}", e);
                    return Err(Error::RAKNET_SocketError);
                }
            };
            ...
            .await
            {
                ...
                Ok(p) => p,
                Err(_) => return Err(Error::RAKNET_SocketError),
            };

            if let Ok(p) = read_packet_pong(&buf) {
                return Ok((
                    p.time - packet.time,
                    String::from(String::from_utf8_lossy(&*p.motd)),
                ));
            };
    ...
    ///
    /// Except Reliability::ReliableOrdered, all other reliability packets must be less than MTU - 60 (default 1340 bytes), otherwise Error::RAKNETPacketSizeExceedMTU will be returned
    ///
    /// # Example
    /// ```ignore
    /// let socket = RaknetSocket::connect("127.0.0.1:19132".parse().unwrap()).await.unwrap();
    /// socket.send(&[0xfe], Reliability::ReliableOrdered).await.unwrap();
    /// ```
    pub async fn send(&self, buf: &[u8], r: Reliability) -> ServerResult<()> {
        if buf.is_empty() {
            return Err(Error::RAKNET_PacketHeaderError);
        }
        ...
        if self.close_notifier.is_closed() {
            return Err(Error::RAKNET_ConnectionClosed);
        }
    ...
    /// ```
    pub async fn flush(&self) -> ServerResult<()> {
        loop {
            {
                if self.close_notifier.is_closed() {
                    return Err(Error::RAKNET_ConnectionClosed);
    ...
    /// }
    /// ```
    pub async fn recv(&self) -> ServerResult<Vec<u8>> {
    ...
    /// ```
    pub fn peer_addr(&self) -> ServerResult<SocketAddr> {
        Ok(self.peer_addr)
    }
    ...
    pub fn raknet_version(&self) -> ServerResult<u8> {
