...
use crate::{
    error::{RaknetError, RaknetResult},
...
impl RaknetSocket {
    ...
    pub async fn connect(addr: &SocketAddr) -> RaknetResult<Self> {
        Self::connect_with_version(addr, RAKNET_PROTOCOL_VERSION).await
    }

    pub async fn connect_with_version(addr: &SocketAddr, raknet_version: u8) -> RaknetResult<Self> {
    ...
    /// ```
    pub async fn close(&self) -> RaknetResult<()> {
    ...
    /// assert!((0..10).contains(&latency));
    /// ```
    pub async fn ping(addr: &SocketAddr) -> RaknetResult<(i64, String)> {
        let s = match UdpSocket::bind("0.0.0.0:0").await {
            Ok(p) => p,
            Err(_) => return Err(RaknetError::BindAdressError),
        };
    ...
    /// socket.send(&[0xfe], Reliability::ReliableOrdered).await.unwrap();
    /// ```
    pub async fn send(&self, buf: &[u8], r: Reliability) -> RaknetResult<()> {
        if buf.is_empty() {
    ...
    /// ```
    pub async fn flush(&self) -> RaknetResult<()> {
        loop {
            {
                if self.close_notifier.is_closed() {
                    return Err(RaknetError::ConnectionClosed);
    ...
    /// }
    /// ```
    pub async fn recv(&self) -> RaknetResult<Vec<u8>> {
    ...
    /// ```
    pub fn peer_addr(&self) -> RaknetResult<SocketAddr> {
        Ok(self.peer_addr)
    }
    ...
    /// ```
    pub fn local_addr(&self) -> RaknetResult<SocketAddr> {
        Ok(self.local_addr)
    }

    /// return the raknet version used by this connection.
    pub fn raknet_version(&self) -> RaknetResult<u8> {
