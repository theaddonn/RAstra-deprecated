...
use crate::error::{RaknetError, RaknetResult};
use crate::packet::*;
use crate::utils::*;
...
impl RaknetListener {
    ...
    /// ```
    pub async fn bind(sockaddr: &SocketAddr) -> Result<Self, Error> {
    ...
    /// ```
    pub async fn from_std(s: std::net::UdpSocket) -> RaknetResult<Self> {
    ...
    /// ```
    pub async fn accept(&mut self) -> RaknetResult<RaknetSocket> {
        if !self.listened {
    ...
    /// socket.close().await;
    /// ```
    pub async fn close(&mut self) -> RaknetResult<()> {
    ...
    /// ```
    pub fn set_full_motd(&mut self, motd: String) -> RaknetResult<()> {
        self.motd = motd;
        Ok(())
    }

    pub async fn get_peer_raknet_version(&self, peer: &SocketAddr) -> RaknetResult<u8> {
