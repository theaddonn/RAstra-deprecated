...
impl Reliability {
    ...
    pub fn from(flags: u8) -> ServerResult<Self> {
        match flags {
            ...
            _ => Err(Error::RAKNET_IncorrectReliability),
        }
    }
}
...
impl FrameSetPacket {
    ...
    pub fn _deserialize(buf: Vec<u8>) -> ServerResult<(Self, bool)> {
    ...
    pub fn is_reliable(&self) -> ServerResult<bool> {
    ...
    pub fn is_ordered(&self) -> ServerResult<bool> {
        let r = Reliability::from((self.flags & 224) >> 5)?;
        Ok(matches!(
            r,
    ...
    pub fn is_sequenced(&self) -> ServerResult<bool> {
        let r = Reliability::from((self.flags & 224) >> 5)?;
        Ok(matches!(
            r,
    ...
    pub fn reliability(&self) -> ServerResult<Reliability> {
        Reliability::from((self.flags & 224) >> 5)
    }

    pub fn _size(&self) -> ServerResult<usize> {
        let mut ret = 0;
        // id
        ret += 1;
        // sequence number
...
impl FrameVec {
    pub fn new(buf: Vec<u8>) -> ServerResult<Self> {
        let mut ret = Self {
            id: 0,
            sequence_number: 0,
            frames: vec![],
    ...
    pub fn _serialize(&self) -> ServerResult<Vec<u8>> {
        let mut writer = PacketWriter::new();
...
impl RecvQ {
    ...
    pub fn insert(&mut self, frame: FrameSetPacket) -> ServerResult<()> {
...
impl SendQ {
    ...
    pub fn insert(&mut self, reliability: Reliability, buf: &[u8]) -> ServerResult<()> {
        match reliability {
            Reliability::Unreliable => {
                // 60 = max framesetpacket length(27) + udp overhead(28) + 5 ext
                if buf.len() > (self.mtu - 60).into() {
                    return Err(Error::RAKNET_PacketSizeExceedMTU);
                }
