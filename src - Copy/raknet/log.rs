...
macro_rules! raknet_log_debug {
    ($($arg:tt)*) => ({
        if $crate::raknet::log::ENABLE_RAKNET_LOG.load(std::sync::atomic::Ordering::Relaxed) & 1 != 0 {
            let now = $crate::raknet::utils::cur_timestamp_millis();
            let msg = format!($($arg)*);
...
macro_rules! raknet_log_error {
    ($($arg:tt)*) => ({
        if $crate::raknet::log::ENABLE_RAKNET_LOG.load(std::sync::atomic::Ordering::Relaxed) & 2 != 0 {
            let now = $crate::raknet::utils::cur_timestamp_millis();
            let msg = format!($($arg)*);
...
macro_rules! raknet_log_info {
    ($($arg:tt)*) => ({
        if $crate::raknet::log::ENABLE_RAKNET_LOG.load(std::sync::atomic::Ordering::Relaxed) & 4 != 0 {
            let now = $crate::raknet::utils::cur_timestamp_millis();
            let msg = format!($($arg)*);
